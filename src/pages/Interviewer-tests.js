...
export default function InterviewerTests() {
  ...
  const deleteTest = (_id) => {
    setLoading(true);
    axios
      .delete(`${API_URL}/api/meeting/tests/delete-specific/${_id}`, {
        headers: { authorization: accessToken },
      })
      .then((response) => {
        console.log('response', response);
        setResponse({ status: 200, message: 'Test Deleted Successfully' });
        handleNotification();
        handleRender();
      })
      .catch((error) => {
        setLoading(false);
        setResponse({ status: 404, message: 'Cannot Delete Test' });
        handleNotification();
        console.log(error);
      });
  };
  ...
  return (
    <Page title="Meetings">
                        ...
                        return (
                          ...
                          >
                            ...
                            <TableCell align="right">
                              <TestMoreMenu test={row} deleteTest={deleteTest} />
                            </TableCell>
                          </TableRow>
                        );
                      })
      ...
      {openModal ? <TestModal open={openModal} handleClose={handleModal} handleRender={handleRender} /> : null}
