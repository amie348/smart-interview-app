...
export default function CandidateMeetingMoreMenu({ meeting }) {
  ...
  const comparePassword = () => {
    ...
    if (password === meeting.password) {
      navigate(`/candidate-call/${meeting._id}/${meeting.connection}`);
    } else {
      setError(true);
    }
    setLoading(false);
  ...
  return (
    <>
      ...
      >
        {meeting.status !== 'STARTED' || !moment(meeting.expiryDate).isAfter(moment()) ? null : (
          <MenuItem sx={{ color: 'success.main' }} onClick={handleOpenModal}>
            ...
            <ListItemText primary="Join Meeting" primaryTypographyProps={{ variant: 'body2' }} />
          </MenuItem>
        )}
      </Menu>
      <Modal
